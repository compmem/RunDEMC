...
def boxcox(x, lambdax, shift=0.0):
    ...
    return np.log(x+shift) if np.abs(lambdax) < 1.0e-5 else ((x+shift)**lambdax - 1.0) / lambdax
...
def find_good_ind(dat, sd=3.0, verbose=True, max_iter=100, shift_start=0.0):
    ...
    ind = np.ones(len(dat), dtype=np.bool)
    tot_removed = 0
    ...
    for i in range(max_iter):
        ...
        tot_removed += removed
        if verbose:
            print('Removed %d out of %d, lambdax = %f, shift = %f' % 
    ...
    if verbose:
        print('Removed %d in total out of %d in %d iterations.' %
